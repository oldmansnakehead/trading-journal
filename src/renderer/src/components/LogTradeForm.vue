...
async function handleSubmit() {
  ...
  if (!form.value.rrTypeId) {
    showToast('error', 'กรุณาเลือก RR Type')
    return
  }
  if (!form.value.entryDate || !form.value.entryTime) {
    showToast('error', 'กรุณากรอกวันและเวลา Entry')
    return
  }
  if (!parseThaiDisplayDate(form.value.entryDate)) {
    showToast('error', 'รูปแบบวันที่ Entry ต้องเป็น dd/mm/yyyy (ค.ศ.)')
    return
  }
  if (form.value.exitDate && !parseThaiDisplayDate(form.value.exitDate)) {
    showToast('error', 'รูปแบบวันที่ Exit ต้องเป็น dd/mm/yyyy (ค.ศ.)')
    return
  }
  ...
  try {
    ...
    for (const url of imageUrls) {
      try {
        ...
        showToast('error', `ลิงก์รูปไม่ถูกต้อง: ${url}`)
    ...
    showToast('success', 'บันทึกการเทรดเรียบร้อย!')
    emit('submitted')
  } catch (err) {
...
<template>
  <div class="log-trade-form">
    ...
    <!-- Toast -->
    <Teleport to="body">
      <div v-if="toast.show" :class="['ltf-toast', `ltf-toast--${toast.type}`]">
        {{ toast.msg }}
      </div>
    </Teleport>
  </div>
...
.ltf-toast {
  ...
  pointer-events: none;
  animation: ltf-toast-in 0.2s ease;
}
.ltf-toast--success {
  background: #166534;
  color: #bbf7d0;
